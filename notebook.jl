...
# ╔═╡ 536d9508-a717-11ed-283d-1121b4a2d7f0
using Plots

# ╔═╡ 00ae878f-b3bd-43ee-a9dd-7570e0397cdd
using QuantumInformation

# ╔═╡ 10896f9e-3776-4d04-bc7d-75aa3a15e1f0
using SparseArrays

# ╔═╡ 3dc8973f-50a1-4063-82fb-e976d8930c93
using PlutoUI

# ╔═╡ c93822be-43fb-4e7f-b57c-27a26afb636b
using LinearAlgebra

# ╔═╡ cc8bbcc7-65ad-4a31-86e8-eba49b261928
md"""
!!! info ""
	`maxtime` is the time to run the quantum walk. Don't set to more than 100 on laptop.
"""

# ╔═╡ 143830e6-12cd-478b-b4d1-1397c00a22e5
maxtime = 75

# ╔═╡ 1f83de17-8c63-4d71-841c-1c502889a96f
r0 = b0 = 10

# ╔═╡ 61d571f7-ceea-4a27-a7eb-069ef5afcf90
statespace = maxtime + max(r0, b0)

# ╔═╡ 5bdc18d7-dc69-497e-aafb-39f929035124
Crb(r, b) = 1/√(r+b) * [√r √b; √b -√r]

# ╔═╡ c0d54c3f-f019-4cbc-b8f7-8a777c598af0
coin_op = sum(Iterators.product(1:statespace, 1:statespace)) do (r,b)
	sparse(proj(ket(r, statespace))) ⊗ 
	sparse(proj(ket(b, statespace))) ⊗
	Crb(r,b)
end

# ╔═╡ 23364a04-c874-4170-9925-620e99f5d784
S = sum(Iterators.product(1:statespace-1, 1:statespace-1)) do (r,b)
	(
		sparse(ket(r+1, statespace)⊗ket(b, statespace)⊗ket(1, 2)) *
		(bra(r, statespace)⊗bra(b, statespace)⊗bra(1, 2))
	) + 
	(
		sparse(ket(r, statespace)⊗ket(b+1, statespace)⊗ket(2, 2)) *
		(bra(r, statespace)⊗bra(b, statespace)⊗bra(2, 2))
	)
end

# ╔═╡ 6e82e8a3-1206-4c5f-9b50-e22f33865123
W = S*coin_op

# ╔═╡ 8ca2f108-6e2d-47a2-802b-ec1524b6c33d
md"""
!!! info ""
	`alpha`, `phi` parametrises the initial state of the coin $\sqrt\alpha |R\rangle + e^{i\phi} \sqrt{1 - \alpha} |B\rangle$
"""

# ╔═╡ 88c00215-cb75-4773-af5f-fc961071b5fb
@bind α Slider(0:0.001:1, show_value=true)

# ╔═╡ bdf2a408-145b-4685-b656-c52a070c0022
@bind ϕ Slider(0:0.001:1, show_value=true)

# ╔═╡ 9e5145b7-f171-4ad1-a685-ac9f784705df
init = ket(r0, statespace) ⊗ 
	ket(r0, statespace) ⊗ 
	(√α * ket(1, 2) + cis(ϕ)*√(1-α)*ket(2, 2))

# ╔═╡ 9bd2d900-5fb2-4da8-8d48-fad3b75f5c45
k = accumulate(1:maxtime, init=init) do old, _
	W*old
end

# ╔═╡ b30a7b87-3f7c-4d08-b841-32e7fbac06e5
ls = map(k) do l
	ptrace(ptrace(l, [statespace*statespace, 2], 2), [statespace, statespace], 2)
end

# ╔═╡ b6183672-d911-417d-8f5a-ec751fb14be3
readouts = ls .|> diag |> real

# ╔═╡ e2b7f619-ded6-4d4c-8506-1aa7e94b1e34
@bind t Slider(1:maxtime, show_value=true)

# ╔═╡ cd4fb2f1-4947-4446-b07b-16dbe107776e
readouts[t] |> plot

# ╔═╡ 25b02083-adc6-4597-ae4b-706f5b21b43c
map(1:statespace) do r
	sum(Iterators.product(1:statespace, 1:2)) do (b, c)
		k[end][(r-1)*statespace*2 + (b-1)*statespace + c]
	end
end

# ╔═╡ 28a02e27-4352-4515-a3a0-0187037778b1
[0, 3] ⊗ [1, 1, 2] ⊗ [3, 4]
...
manifest_format = "2.0"
project_hash = "1f470c95efdc84e6660d05a94ea48b515aad6107"
